...
import static com.openhtmltopdf.layout.BoxBuilder.insertGeneratedContent;
...
/**
 * A block box as defined in the CSS spec. It also provides a base class for other kinds of block content (for example table rows or cells). See {@link ContentType}
 */
public class BlockBox extends Box {
	...
	/**
	 * What type of direct child content this block box contains. <br>
	 * <br>
	 * NOTE: A {@link BlockBox} can only contain inline or block content (not both) as direct children. If this constraint is not met by the original document, the {@link BoxBuilder} will insert {@link AnonymousBlockBox} with inline content.
	 */
	public enum ContentType {
		/**
		 * The box builder has not yet run to create our child boxes. The box builder can be run with {@link BlockBox#ensureChildren(LayoutContext)}.
		 */
		UNKNOWN,

		/**
		 * This block box contains inline content in the {@link BlockBox#getInlineContent()} property. If it has also been laid out it will contain children in {@link Box#getChildren()} and associated methods. Children will be only {@link LineBox} objects.
		 */
		INLINE,

		/**
		 * This block box's direct children consist only of {@link BlockBox} and subclassed objects. The method {@link BlockBox#setInlineContent(List)} must not be used with block content.
		 */
		BLOCK,
	...
	public void createMarkerData(LayoutContext c) {
		...
		if (markerStyle != null && markerStyle.hasProperty(CSSName.CONTENT)) {
			...
			if (listStyle == IdentValue.CIRCLE || listStyle == IdentValue.SQUARE || listStyle == IdentValue.DISC || listStyleType != null) {
	...
	private MarkerData.TextMarker makeTextMarker(LayoutContext c, IdentValue listStyle) {
		...
		int w = c.getTextRenderer().getWidth(c.getFontContext(), getStyle().getFSFont(c), text);
	...
	public void positionAbsolute(CssContext cssCtx, int direction) {
		...
		if ((direction & POSITION_HORIZONTALLY) != 0) {
			...
			if (!style.isIdent(CSSName.LEFT, IdentValue.AUTO)) {
				...
				setX(boundingBox.width - (int) style.getFloatPropertyProportionalWidth(CSSName.RIGHT, getContainingBlock().getContentWidth(), cssCtx) - getWidth());
			}
		...
		if ((direction & POSITION_VERTICALLY) != 0) {
			...
			if (!style.isIdent(CSSName.TOP, IdentValue.AUTO)) {
				...
				setY(boundingBox.height - (int) style.getFloatPropertyProportionalWidth(CSSName.BOTTOM, cbContentHeight, cssCtx) - getHeight());
			}
		...
		if ((direction & POSITION_VERTICALLY) != 0 && getStyle().isTopAuto() && getStyle().isBottomAuto()) {
	 ...
	 *
	 * on a block element you can force a pagebreak before this block, if not enough space (e.g. 5cm in this case) is remaining on the current page for the block.
	 *
	 * @return true if a pagebreak is needed before this block because there is not enough space left on the current page.
	 */
	...
	public void positionAbsoluteOnPage(LayoutContext c) {
		if (c.isPrint() && (getStyle().isForcePageBreakBefore() || isNeedPageClear() || isPageBreakNeededBecauseOfMinHeight(c))) {
	...
	private int calcPinnedContentWidth(CssContext c) {
		if (!getStyle().isIdent(CSSName.LEFT, IdentValue.AUTO) && !getStyle().isIdent(CSSName.RIGHT, IdentValue.AUTO)) {
			...
			int left = (int) getStyle().getFloatPropertyProportionalTo(CSSName.LEFT, paddingEdge.width, c);
			int right = (int) getStyle().getFloatPropertyProportionalTo(CSSName.RIGHT, paddingEdge.width, c);
	...
	private int calcPinnedHeight(CssContext c) {
		if (!getStyle().isIdent(CSSName.TOP, IdentValue.AUTO) && !getStyle().isIdent(CSSName.BOTTOM, IdentValue.AUTO)) {
			...
			int top = (int) getStyle().getFloatPropertyProportionalTo(CSSName.TOP, paddingEdge.height, c);
			int bottom = (int) getStyle().getFloatPropertyProportionalTo(CSSName.BOTTOM, paddingEdge.height, c);
	...
	private void createReplaced(LayoutContext c) {
		...
		if (re == null) {
			...
			if (minWidth > cssWidth && minWidth > 0) {
				cssWidth = minWidth;
			}

			if (minHeight > cssHeight && minHeight > 0) {
				cssHeight = minHeight;
			}
	...
	private void sizeReplacedElement(LayoutContext c, ReplacedElement re) {
		...
		if (!getStyle().isMaxWidthNone() && (intrinsicWidth > getCSSMaxWidth(c) || cssWidth > getCSSMaxWidth(c))) {
			cssWidth = getCSSMaxWidth(c);
			usedMaxWidth = true;
		}

		// Clamp w to min-width if required.
		if (cssWidth >= 0 && minWidth > 0 && cssWidth < minWidth) {
			cssWidth = minWidth;
			usedMinWidth = true;
		}

		// Clamp h to max-height if required.
		if (!getStyle().isMaxHeightNone() && (intrinsicHeight > getCSSMaxHeight(c) || cssHeight > getCSSMaxHeight(c))) {
			cssHeight = getCSSMaxHeight(c);
			usedMaxHeight = true;
		}

		// Clamp h to min-height if required.
		if (cssHeight >= 0 && minHeight > 0 && cssHeight < minHeight) {
			cssHeight = minHeight;
			usedMinHeight = true;
		}
		...
		boolean useExact = (haveExactDims && !usedMaxHeight && !usedMaxWidth && !usedMinWidth && !usedMinHeight);
	...
	protected void calcDimensions(LayoutContext c, int cssWidth) {
		if (!isDimensionsCalculated()) {
			...
			if (cssWidth != -1 && !isAnonymous() && (getStyle().isIdent(CSSName.MARGIN_LEFT, IdentValue.AUTO) || getStyle().isIdent(CSSName.MARGIN_RIGHT, IdentValue.AUTO)) && getStyle().isNeedAutoMarginResolution()) {
	...
	private void calcExtraPageClearance(LayoutContext c) {
		if (c.isPageBreaksAllowed() && c.getExtraSpaceTop() > 0 && (getStyle().isSpecifiedAsBlock() || getStyle().isListItem())) {
	...
	/**
	 * Checks if this box established a block formatting context and if so removes the last bfc from the stack. See also {@link #checkPushBfc(LayoutContext)}
	 */
	...
	/**
	 * Checks if this box establishes a block formatting context and if so creates one and pushes it to the stack of bfcs. See also {@link #checkPopBfc(LayoutContext)}
	 */
	...
	/**
	 * TERMINOLOGY: Orphans refer to the number of lines of content in this box before the first page break. Widows refer to the number of lines of content on the last page. <br>
	 * <br>
	 * METHOD AIM: This method aims (but can not guarantee) to satisfy the <code>orphans</code> and <code>widows</code> CSS properties. Each of these provide a number specifying a minimum number of content lines. <br>
	 * <br>
	 * HOW: By inserting page breaks, either before this box or between certain lines in this box. <br>
	 * <br>
	 * PREREQUISITES: That the content of this box is <code>CONTENT_INLINE</code> and layout has been done on this box. This means that the children of this box will consist entirely of LineBox objects.
	 */
	...
	/**
	 * Inline content is created by the box builder. It is important to note that the inline content here is stored in the pre-layout state. Ie. It has not been flowed out into {@link LineBox} and {@link InlineLayoutBox} objects but is stored as {@link InlineBox} and block boxes that are laid out inline such as inline-block and inline-table. <br>
	 * <br>
	 * During layout inline-content is laid out into lines and so on but the inline content is left untouched to be able to run layout multiple times to satisfy constraints. <br>
	 * <br>
	 * This method should be called with {@link #setChildrenContentType(ContentType)} set to {@link ContentType#INLINE} as block boxes can not contain mixed content.
	 */
	...
	protected void collapseMargins(LayoutContext c) {
		if (!isTopMarginCalculated() || !isBottomMarginCalculated()) {
			...
			if (!isTopMarginCalculated() && !isBottomMarginCalculated() && isVerticalMarginsAdjoin(c)) {
				MarginCollapseResult collapsedMargin = _pendingCollapseCalculation != null ? _pendingCollapseCalculation : new MarginCollapseResult();
		...
		if (!isAnonymous()) {
			if (!getStyle().isAutoWidth()) {
				if (shrinkingToFit && !getStyle().isAbsoluteWidth()) {
					...
					int result = (int) getStyle().getFloatPropertyProportionalWidth(CSSName.WIDTH, getContainingBlock().getContentWidth(), c);
	...
	protected int getCSSFitToWidth(CssContext c) {
		if (!isAnonymous()) {
			if (!getStyle().isIdent(CSSName.FS_FIT_IMAGES_TO_WIDTH, IdentValue.AUTO)) {
				int result = (int) getStyle().getFloatPropertyProportionalWidth(CSSName.FS_FIT_IMAGES_TO_WIDTH, getContainingBlock().getContentWidth(), c);
				return result >= 0 ? result : -1;
			}
		}
		...
		if (!isAnonymous()) {
			if (!isAutoHeight()) {
				if (getStyle().hasAbsoluteUnit(CSSName.HEIGHT)) {
					...
					return (int) getStyle().getFloatPropertyProportionalHeight(CSSName.HEIGHT, ((BlockBox) getContainingBlock()).getCSSHeight(c), c);
				}
			}
		}
	...
	public boolean isAutoHeight() {
		if (getStyle().isAutoHeight()) {
			...
			if (cb.isStyled() && (cb instanceof BlockBox)) {
				...
				return !(cb instanceof BlockBox) || !cb.isInitialContainingBlock();
		}
	}
	...
	private int getContainingBlockCSSHeight(CssContext c) {
		if (!getContainingBlock().isStyled() || getContainingBlock().getStyle().isAutoHeight()) {
			return 0;
		} else {
			if (getContainingBlock().getStyle().hasAbsoluteUnit(CSSName.HEIGHT)) {
				return (int) getContainingBlock().getStyle().getFloatPropertyProportionalTo(CSSName.HEIGHT, 0, c);
			} else {
				return 0;
			}
		}
	...
	protected int getAvailableWidth(LayoutContext c) {
		if (!getStyle().isAbsolute()) {
			...
			if (!getStyle().isIdent(CSSName.LEFT, IdentValue.AUTO)) {
				left = (int) getStyle().getFloatPropertyProportionalTo(CSSName.LEFT, getContainingBlock().getContentWidth(), c);
			}

			if (!getStyle().isIdent(CSSName.RIGHT, IdentValue.AUTO)) {
				right = (int) getStyle().getFloatPropertyProportionalTo(CSSName.RIGHT, getContainingBlock().getContentWidth(), c);
			}
	...
	private void recalcMargin(LayoutContext c) {
		...
		// in this case.
		if (!isTopMarginCalculated() && styleMargin.top() != workingMargin.top()) {
			setMarginTop(c, (int) styleMargin.top());
		}

		if (!isBottomMarginCalculated() && styleMargin.bottom() != workingMargin.bottom()) {
	...
	private void calcMinMaxCSSMinMaxWidth(LayoutContext c, RectPropertySet margin, BorderPropertySet border, RectPropertySet padding) {
		int cssMinWidth = getCSSMinWidth(c);
		if (cssMinWidth > 0) {
			cssMinWidth += (int) margin.left() + (int) border.left() + (int) padding.left() + (int) margin.right() + (int) border.right() + (int) padding.right();
		...
		if (!getStyle().isMaxWidthNone()) {
			int cssMaxWidth = getCSSMaxWidth(c);
			cssMaxWidth += (int) margin.left() + (int) border.left() + (int) padding.left() + (int) margin.right() + (int) border.right() + (int) padding.right();
	...
	private void calcMinMaxWidthInlineChildren(LayoutContext c) {
		int textIndent = (int) getStyle().getFloatPropertyProportionalWidth(CSSName.TEXT_INDENT, getContentWidth(), c);
	...
	public boolean checkPageContext(LayoutContext c) {
		if (!getStyle().isIdent(CSSName.PAGE, IdentValue.AUTO)) {
			...
			if (!pageName.equals(c.getPageName()) && isInDocumentFlow() && (shouldBeReplaced() || isContainsInlineContent(c))) {
