...
import com.openhtmltopdf.jhtml.swing.dither.DefaultDither;
...
public class SimpleDitherStrategy implements BaseDither {
	 ...
	 */
	public static class Builder implements DefaultDither{
		 ...
		 */
		private BufferedImage doDitherCore(BufferedImage src, ColorMode colorMode, DitherKernel kernel, byte[] outHardwareIndexBytes) {
		 ...
		 */
		private BufferedImage runBayer8x8(BufferedImage src, ColorMode colorMode, byte[] outHardwareIndexBytes) {
			int width = src.getWidth();
			int height = src.getHeight();
		 ...
		 */
		private int findNearestIndex(PaletteCache pc, int r, int g, int b) {
			int bestIdx = 0;
		 ...
		 */
		private short clampShort(short val, int min, int max) {
			if (val < min) {
				return (short) min;
			}
			if (val > max) {
